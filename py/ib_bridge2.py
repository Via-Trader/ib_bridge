...
def bracket_order(action, quantity, entry_price, stop_loss_offset, limit_price_offset):
    """Create and return bracket orders (parent, take-profit, stop-loss) with unique IDs."""
    parent_order_id = get_next_order_id()  # Get a unique ID for the parent order

    # Parent (entry) order
    parent = LimitOrder(action=action, totalQuantity=quantity, lmtPrice=entry_price)
    parent.orderId = parent_order_id
    parent.transmit = False  # Parent order not transmitted yet

    # Take-profit order
    take_profit = LimitOrder(
        action='SELL' if action == 'BUY' else 'BUY',
        totalQuantity=quantity,
        lmtPrice=entry_price + limit_price_offset if action == 'BUY' else entry_price - limit_price_offset
    )
    take_profit.orderId = get_next_order_id()  # Get a unique ID for the take-profit order
    take_profit.parentId = parent_order_id  # Link to parent order
    take_profit.transmit = False  # Still not transmitting

    # Stop-loss order
    stop_loss = StopOrder(
        action='SELL' if action == 'BUY' else 'BUY',
        totalQuantity=quantity,
        stopPrice=entry_price - stop_loss_offset if action == 'BUY' else entry_price + stop_loss_offset
    )
    stop_loss.orderId = get_next_order_id()  # Get a unique ID for the stop-loss order
    stop_loss.parentId = parent_order_id  # Link to parent order
    stop_loss.transmit = True  # This order transmits the entire chain
...
def process_trade_idea(trade, ib, config):
    ...
    entry_price_offset = float(order_config["entry_price_offset"])
    stop_loss_offset = float(order_config["stop_loss_price"])
    limit_price_offset = float(order_config["limit_price"])
    ...
    adjusted_entry_price = latest_price + entry_price_offset

    print(f"Latest Price: {latest_price}")
    print(f"Adjusted Entry Price: {adjusted_entry_price}, Stop Loss: {adjusted_entry_price - stop_loss_offset}, Target Price: {adjusted_entry_price + limit_price_offset}")
    ...
    bracket_orders = bracket_order(
        action, quantity, adjusted_entry_price, stop_loss_offset, limit_price_offset
    )
