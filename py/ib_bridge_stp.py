...
def bracket_order(action, quantity, stop_price, stop_loss_price, limit_price):
    ...
    parent = StopOrder(action=action, totalQuantity=quantity, stopPrice=stop_price)
...
def process_trade_idea(trade, ib, config):
    ...
    stop_price_offset = float(order_config["stop_price_offset"])
    limit_price_offset = float(order_config["limit_price_offset"])
    ...
    bracket_orders = bracket_order(
        action, quantity, adjusted_entry_price, abs(stop_loss_offset), limit_price_offset
    )
