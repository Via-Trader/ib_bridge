...
def process_trade_idea(trade, ib, config):
    ...
    if action == 'BUY':
        stop_price = latest_price + stop_offset
        limit_price = latest_price + limit_offset
        stop_loss_price = stop_price - stop_loss_offset
        take_profit_price = stop_price + take_profit_offset
    elif action == 'SELL':
        stop_price = latest_price - stop_offset
        limit_price = latest_price - limit_offset
        stop_loss_price = stop_price + stop_loss_offset
